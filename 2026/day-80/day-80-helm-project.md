...
We found the "Deadlock" in the code!

Look at the `annotations` in the YAML code:
...
**🔍 The Problem**
In Helm, a `pre-install` hook runs **before** any other resources in the chart (like MySQL Deployment or Service) are even created.

  1. Helm starts the `db-ready` Job.

  2. The Job runs `nc -z my-bankapp-mysql 3306`.

  3. **The Deadlock:** The `my-bankapp-mysql` service doesn't exist yet because Helm is waiting for this Job to finish before it creates it.

  4. The Job loops forever (or until the 10-minute Helm timeout) because it's waiting for something that hasn't been born yet.

**🛠️ The Solution: Change the Hook Type**

To fix this properly without using `--no-hooks` every time, we need to move this logic from a `pre-install` Job to an `initContainer` inside BankApp deployment, or change the hook type.

  **The "Quick Fix" (Use Post-Install)**

If we change the hook to `post-install`, Helm will create the Database first, and then run the check.
Change line 11 in `bankapp/templates/pre-install-job.yaml`:
...
metadata:
  ...
  labels:
    {{- include "bankapp.labels" . | nindent 4 }}
  annotations:
    "helm.sh/hook": post-install # Updated to post-install as discussed above
    "helm.sh/hook-weight": "0"
    "helm.sh/hook-delete-policy": before-hook-creation
spec:
  template:
