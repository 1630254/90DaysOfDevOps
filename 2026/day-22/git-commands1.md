...
- `git diff` →  Displays the differences between files in your working directory and the staging area (or between commits).
...
- `git stash` → Temporarily saves your uncommitted changes and clears the working directory
...
- `git cherry-pick <commit_hash>` → Applies the changes from a specific commit onto your current branch.
